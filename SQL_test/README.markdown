...
Installation
    ...
    Deploy the Stored Procedure: Run the SQL script using psql or a database client:
    ...
    Verify Installation: Check the procedure exists:
...
Recommended Test Scenarios
    ...
    Frequent Calls: Loop 1000 times with identical parameters to check plan cache reuse.
    Skewed Data: Insert 1M rows with 99% identical p_dimension_column values.
    Custom Access Method: Test with a hypothetical table access method extension.
    Memory Pressure: Run with vm.overcommit_memory = 2 and low RAM.
    Parallel Queries: Set max_parallel_workers_per_gather = 4 with a large table.
    JIT Compilation: Set enable_jit = on with small/large datasets.
    RLS: Enable row-level security on p_table_name.
    Extensions: Test with TimescaleDB or Citus.
    TOASTed Columns: Use a TEXT column with 1MB+ values for p_dimension_column.
    High Cardinality: Test with p_dimension_column having 500,000 unique values.
...
Contributions are welcome! Please follow these steps:
    ...
    Open a pull request with a clear description and test cases.
